...
function tambahpenarikan($data){
	...
	$totalSaldo = isset($ambilSaldo['totalSaldo']) ? $ambilSaldo['totalSaldo'] : 0;
...
function ubah($data) {
    ...
    if (isset($_FILES['gambar']) && $_FILES['gambar']['error'] !== 4) {
        // Gambar diupload, maka upload gambar baru
        $gambar = uploadGambar();
    } else {
        // Gambar tidak diupload, gunakan gambar lama
        $gambar = $gambarlama;
    }
    ...
    // Buat query hanya untuk update data yang berubah
    $query = "UPDATE users SET
                namaUser = '$namaUser',
                gambar = '$gambar',  -- Gambar bisa diperbarui, jika ada file baru
                nik = '$nik',
                alamat = '$alamat',
                telepon = '$telepon',
                username = '$username',
...
function editPengguna($data) {
	...
	$gambarlama = htmlspecialchars((isset($data["gambarlama"])) ? $data["gambarlama"] : '');
	// cek apakah user milih gambar baru atau tidak
	if($_FILES['gambar']['error']===4){
		$gambar = $gambarlama;
	}else{
...
function tambahRecordSaldoBank($data) {
    global $conn;

    // Akumulasi totalPendapatan dari tabel penjualan
    $ambilTotalPendapatan = mysqli_query($conn, "SELECT SUM(totalPendapatan) AS totalPendapatan FROM penjualan");
    $dataPendapatan = mysqli_fetch_array($ambilTotalPendapatan);
    $totalPendapatan = $dataPendapatan['totalPendapatan'];

    // Akumulasi jmlPenarikan dari tabel penarikan
    $ambilTotalPenarikan = mysqli_query($conn, "SELECT SUM(jmlPenarikan) AS totalPenarikan FROM penarikan");
    $dataPenarikan = mysqli_fetch_array($ambilTotalPenarikan);
    $totalPenarikan = $dataPenarikan['totalPenarikan'];

    // Hitung total saldo
    $totalSaldo = $totalPendapatan - $totalPenarikan;

    // Format ID transaksi saldo_bank
    $no = mysqli_query($conn, "SELECT * FROM saldo_bank ORDER BY idTransaksi DESC");
    $noArr = mysqli_fetch_array($no);
    $lastId = ($noArr == null) ? 0 : intval(substr($noArr['idTransaksi'], -3));
    $newId = $lastId + 1;
    $format = "SLD" . str_pad($newId, 3, "0", STR_PAD_LEFT);

    $aksi = "Penyesuaian Saldo";
    $tanggal = stripslashes($data["tanggal"]);
    $aktor = "ADM001"; // Sesuaikan dengan session admin
    $jumlah = $totalPendapatan - $totalPenarikan;

    $query = "INSERT INTO saldo_bank (idTransaksi, aksi, tanggal, aktor, jumlah, totalSaldo)
              VALUES ('$format', '$aksi', '$tanggal', '$aktor', '$jumlah', '$totalSaldo')";
...
function updateTotalSaldo($idPenjualan, $aksi, $data) {
    global $conn;
    ...
    if ($beratpenjualan > $berat) {
        ...
        $sisaStock = $stock; // Tidak ada perubahan
    }
    ...
    // Membuat kode format ID untuk saldo_bank
    $no = mysqli_query($conn, "SELECT * FROM saldo_bank ORDER BY idTransaksi DESC");
    $noArr = mysqli_fetch_array($no);
    if ($noArr == null) {
        $row = "SLD000"; // Jika belum ada transaksi, ID pertama
    } else {
        $row = $noArr[0]; // Ambil ID terakhir
    }
    $takeId = substr($row, -3);
    $lastId = (int)$takeId;
    $newId = $lastId + 1;
    ...
    // Ambil akumulasi totalPendapatan dari tabel penjualan
    $ambilTotalPendapatan = mysqli_query($conn, "SELECT SUM(totalPendapatan) AS totalPendapatan FROM penjualan");
    $dataPendapatan = mysqli_fetch_array($ambilTotalPendapatan);
    $totalPendapatan = $dataPendapatan['totalPendapatan'];

    // Ambil akumulasi jmlPenarikan dari tabel penarikan
    $ambilTotalPenarikan = mysqli_query($conn, "SELECT SUM(jmlPenarikan) AS totalPenarikan FROM penarikan");
    $dataPenarikan = mysqli_fetch_array($ambilTotalPenarikan);
    $totalPenarikan = $dataPenarikan['totalPenarikan'];

    // Hitung totalSaldo: totalPendapatan - totalPenarikan
    $totalSaldo = $totalPendapatan - $totalPenarikan;

    // Tambahkan record baru ke tabel saldo_bank
    $aksi = "Perubahan Saldo"; // Aksi bisa disesuaikan
    $tanggal = stripslashes($data["tanggal"]);
    $aktor = "ADM001"; // ID aktor, bisa disesuaikan dengan session atau parameter lain
    $jumlah = $totalPendapatan - $totalPenarikan; // Hasil perhitungan

    $query = "INSERT INTO saldo_bank
              VALUES
              ('$format', '$aksi', '$tanggal', '$aktor', '$jumlah', '$totalSaldo')";
    mysqli_query($conn, $query);
